 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Digitalus_Service_PayPal_Data_MassPayReceiver
{
	/**
	...
	public function __construct($amount, $rcpt, $rcpt_type = self::RT_EMAIL)
	{
		...
		if (! $this->amount || $this->amount <= 0) {
			...
			throw new Digitalus_Service_PayPal_Data_Exception("MassPay amount must be more than 0");
		}

		if (! self::validateReceiverType($rcpt, $rcpt_type)) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception("'$rcpt' is not a valid receiver ID of type '$rcpt_type'");
		}
	 ...
	 *
	 * @param  string $id
	 * @return Digitalus_Service_PayPal_Data_MassPayReceiver
	 */
	public function setUniqueId($id)
	{
		if (! $id === null && ! preg_match('/^\S{1,30}$/', $id))  {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception("Unique Recipient ID must be 1 - 30 non-space characters");			
		}
	...
	public function setCustomNote($note)
	{
		if (strlen($note) > 4000) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception("Custom note must be less than 4000 bytes long");
		}
	...
	static public function validateReceiverType($value, $type)
	{
		switch ($type) {
			...
			default:
				require_once 'Digitalus/Service/PayPal/Data/Exception.php';
				throw new Digitalus_Service_PayPal_Data_Exception("'$type' is not a valid Receiver ID type");
				break;
		}
	}
}
