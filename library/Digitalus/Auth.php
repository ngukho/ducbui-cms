...
class Digitalus_Auth
{
     ...
     * @var string
     */
    protected  $_userTable = "core_users";
    ...
    protected $_resultRowColumns = array('user_id', 'group_id', 'first_name', 'last_name', 'email', 'role', 'acl_resources');
    ...
    public function __construct($username = null, $password = null)
    {
        ...
        $this->_authAdapter = new Zend_Auth_Adapter_DbTable($this->_dbAdapter, $this->_userTable, $this->_identityColumn, $this->_credentialColumn,'? AND active = 1');
    ...
    public function authenticate()
    {
        ...
        if ($result->isValid()) {
            ...
            $currentUser = $objUsers->find($row->user_id)->current();
            $currentUser->last_login = time();
            $currentUser->save();
            $this->_storage->user = $objUsers->updatePermissionFromGroup($row->user_id);
            return $result;
        }
    }
    ...
    public static function destroy()
    {
        ...
        // Xoa tat ca cac cache voi tag la session_id
        // Day la loai cache tam thoi chi dung cho moi session
