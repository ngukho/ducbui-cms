 ...
 *
 * @package		Use CodeIgniter Library 1.7.2
  * @link		http://codeigniter.com/user_guide/libraries/form_validation.html
 */

class Digitalus_Validation 
{
	...
	function Digitalus_Validation(Zend_Controller_Action $object,$rules = array())
	{	
//		$this->_c_controller = Zend_Controller_Front::getInstance();
		$this->_c_controller = $object;
		$this->_c_view = $object->view;
		
		// Validation rules can be stored in a config file.
		$this->_config_rules = $rules;
		
		Zend_Registry::set('Digitalus_Validation',$this);
	...
	function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		if (is_null($postdata) AND $callback == FALSE)
		{
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules))
			{
				...
				{
					$line = $this->_c_view->getTranslation($type);
...
//					}							
		...
		foreach ($rules As $rule)
		{
			...
			if ($callback === TRUE)
			{
				if ( ! method_exists($this->_c_controller, $rule))
				{ 		
					continue;
				}
				
				// Run the function and grab the result
				$result = $this->_c_controller->$rule($postdata, $param);
			...
			{			
				...
				{
					$line = $this->_c_view->getTranslation($rule);
...
//					}						
	...
	function _translate_fieldname($fieldname)
	{
		...
		if (substr($fieldname, 0, 5) == 'lang:')
		{
			...
			$line = $this->_c_view->getTranslation($line);
			return $line;
