 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Digitalus_Service_PayPal_Data_AuthInfo
{
	/**
	...
	public function __construct($user, $pass, $sig)
	{
		if (! $user) {
			...
			throw new Digitalus_Service_PayPal_Data_Exception('User must be set!');
		}
			
		if (! $pass) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Password must be set!');
		}
			
		if (! ($sig && strlen($sig) == 56)) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Signature must be a 128 bit PayPal API account signature');
		}
