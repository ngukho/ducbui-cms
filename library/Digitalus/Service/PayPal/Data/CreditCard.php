 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Digitalus_Service_PayPal_Data_CreditCard
{
	/**
	 * Card type constants
	 */
	...
	public function __construct($type, $acct, $firstName, $lastName, $expMonth, $expYear, $cvv2 = null, $startDate = null, $issueNumber = null)
	{
		if (! defined(__CLASS__ . '::CC_' . strtoupper($type))) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Unkown card type: "' . $type . '"');
		}
		...
		if (strlen($firstName) < 1 || strlen($firstName) > 25) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('First name must be set and no longer than 25 characters');
		}
		...
		if (strlen($lastName) < 1 || strlen($lastName) > 25) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Last name must be set and no longer than 25 characters');
		}
		...
		if ($expMonth < 1 || $expMonth > 12) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Expiry month must be a valid month number (1 - 12)');
		}
		...
		if (strlen($expYear) != 4) {
			require_once 'Digitalus/Service/PayPal/Data/Exception.php';
			throw new Digitalus_Service_PayPal_Data_Exception('Expiry year must be a 4 digit number');
		}
		...
		if ($cvv2) {
			if ($this->type == self::CC_VISA || $this->type == self::CC_MASTERCARD || $this->type == self::CC_DISCOVER) {
				if (strlen($cvv2) != 3) {
					require_once 'Digitalus/Service/PayPal/Data/Exception.php';
					throw new Digitalus_Service_PayPal_Data_Exception('CVV2 is expected to be exactly 3 digits long');
				}
			    	
			} elseif ($this->type == self::CC_AMEX) {
				if (strlen($cvv2) != 4) {
					require_once 'Digitalus/Service/PayPal/Data/Exception.php';
					throw new Digitalus_Service_PayPal_Data_Exception('CVV2 is expected to be exactly 4 digits long');
				}
			}
		...
		if ($this->type == self::CC_SWITCH || $this->type == self::CC_SOLO) {
			if (strlen($startDate) < 5 || strlen($startDate) > 6) {
				require_once 'Digitalus/Service/PayPal/Data/Exception.php';
				throw new Digitalus_Service_PayPal_Data_Exception('$startDate "' . $startDate . '" does not seem to be a valid startDate value');
			}
			...
			if (! $issueNumber || strlen($issueNumber) > 2) {
				require_once 'Digitalus/Service/PayPal/Data/Exception.php';
				throw new Digitalus_Service_PayPal_Data_Exception('$issueNumber "' . $issueNumber . '" does not seem to be a valid issueNumber value');
			}
