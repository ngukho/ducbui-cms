...
require_once 'Digitalus/Service/PayPal/Data/AuthInfo.php';
require_once 'Digitalus/Service/PayPal/Response.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Digitalus_Service_PayPal
{
	/**
	 ...
	 *
	 * @var Digitalus_Service_PayPal_Data_AuthInfo
	 */
	 ...
	 *
	 * @param Digitalus_Service_PayPal_Data_AuthInfo $auth_info
	 * @param Zend_Http_Client                  $httpClient
	 */
	public function __construct(Digitalus_Service_PayPal_Data_AuthInfo $authInfo, 
	...
	protected function process()
	{
		...
		if (! $httpResponse->isSuccessful()) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('HTTP response from PayPal was not OK: ' . 
				$httpResponse->getStatus());
		}
		
		return new Digitalus_Service_PayPal_Response($httpResponse->getBody());
	}
	 ...
	 * customer.
	 *
	 * @param  Digitalus_Service_PayPal_Data_CreditCard $creditCard
	 * @param  Digitalus_Service_PayPal_Data_Address    $address
	 * @param  float                               $amount
	 * @param  string                              $currency
	 * @param  string                              $remoteAddr
	 * @param  string                              $paymentAction
	 * @return Digitalus_Service_PayPal_Response
	 */
	public function doDirectPayment(Digitalus_Service_PayPal_Data_CreditCard $creditCard, 
		Digitalus_Service_PayPal_Data_Address $address, $amount, $currency = 'USD', 
		$remoteAddr = null, $paymentAction = 'Sale')
	{
		...
		if (! ($paymentAction == 'Sale' || $paymentAction == 'Authorization')) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('Payment Action must be set to either "Sale" or "Authorization"');
		}
			
		$amount = (float) $amount;
		if (! $amount) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('Amount must be a floating-point number bigger than zero');
		}
	...
	public function setExpressCheckout($amount, $returnUrl, $cancelUrl, $params = array())
	{
		...
		if (! Zend_Uri_Http::check($returnUrl)) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('Return URL is not a valid URL');
		}
			
		if (! Zend_Uri_Http::check($cancelUrl)) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('Cancel URL is not a valid URL');
		}
		
		if (! is_array($params)) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception('$params is expected to be an array, ' . gettype($params) . ' given');
		}
	 ...
	 * 
	 * @param  array  $receivers    Array of Digitalus_Service_PayPal_Data_MassPayRceiver objects
	 ...
	 * @return Digitalus_Service_PayPal_Response
	 */
	public function massPay(array $receivers, $rcpttype = Digitalus_Service_PayPal_Data_MassPayReceiver::RT_EMAIL, $emailSubject = '', $currency = 'USD')
	{
		// Make sure we have more than 0 and less than 256 receivers
		if (empty($receivers) || count($receivers) > 255) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception("Number of receivers must be between 1 and 255");
		}
			
		// Validate receiver type
		if ($rcpttype == Digitalus_Service_PayPal_Data_MassPayReceiver::RT_EMAIL) {
			$idfield = 'L_EMAIL';
		} elseif ($rcpttype == Digitalus_Service_PayPal_Data_MassPayReceiver::RT_USERID) {
			$idfield = 'L_RECEIVERID';
		} else {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception("Receiver ID type '$rcpttype' is not valid");
		}

		// Validate currency code
		if (! self::validateCurrencyCode($currency)) {
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception("Currency code '$currency' is not valid");
		}
		...
		if ($emailSubject) {
			if (strlen($emailSubject) > 255) {
				require_once 'Digitalus/Service/PayPal/Exception.php';
				throw new Digitalus_Service_PayPal_Exception("Email Subject must be up to 255 single-byte characters");
			}
		...
		$c = 0;
		foreach ($receivers as $rcpt) { /* @var $rcpt Digitalus_Service_PayPal_Data_MassPayReceiver */
			if (! $rcpt->getReceiverType() == $rcpttype) {
				require_once 'Digitalus/Service/PayPal/Exception.php';
				throw new Digitalus_Service_PayPal_Exception("All receiver ID types must be '$rcpttype', '{$rcpt->getReceiverId()}' is not"); 
			}
	...
	public function getTransactionDetails($transactionId)
	{
		...
		if (! (Zend_Validate::is($transactionId, 'Alnum') &&
			...
			require_once 'Digitalus/Service/PayPal/Exception.php';
			throw new Digitalus_Service_PayPal_Exception("'$transactionId' is noa a valid PayPal transaction ID");   	
		}
